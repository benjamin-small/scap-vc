...
impl Capturer {
    ...
    ///
    /// scap-vc patch: returns `Err` instead of panicking when the underlying
    /// stream fails to start (e.g. invalid configuration, vanished window).
    pub fn start_capture(&mut self) -> Result<(), String> {
        self.engine.start()
    }

    /// Stop the capturer (scap-vc patch: `Result` instead of panic).
